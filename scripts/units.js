/* ============================================================
   CalcPro — units.js
   Unit Conversion: Temperature, Length, Weight, Volume, Area,
   Speed, Data, Time, Energy, Pressure
   ============================================================ */
...
const UNIT_CATEGORIES = {
  temperature: {
    ...
    formula(from, to) {
      ...
      return map[`${from}→${to}`] || `Konversi ${from} → ${to}`;
    }
  },

  length: {
    label: 'Panjang',
    units: ['Meter (m)', 'Kilometer (km)', 'Centimeter (cm)', 'Millimeter (mm)', 'Inch (in)', 'Feet (ft)', 'Yard (yd)', 'Mile (mi)', 'Nautical Mile (nmi)', 'Light Year (ly)', 'Angstrom (Å)'],
    ...
    formula(from, to) { return `1 ${from} = ${(this.toBase[from]/this.toBase[to]).toPrecision(6)} ${to}`; }
  },

  weight: {
    label: 'Berat / Massa',
    units: ['Kilogram (kg)', 'Gram (g)', 'Milligram (mg)', 'Ton (t)', 'Pound (lb)', 'Ounce (oz)', 'Carat (ct)', 'Grain (gr)', 'Stone (st)'],
  ...
  volume: {
    label: 'Volume',
    units: ['Liter (L)', 'Milliliter (mL)', 'Cubic Meter (m³)', 'Cubic cm (cm³)', 'Gallon US (gal)', 'Quart (qt)', 'Pint (pt)', 'Cup (cup)', 'Fluid Ounce (fl oz)', 'Tablespoon (tbsp)', 'Teaspoon (tsp)'],
  ...
  area: {
    label: 'Luas',
    units: ['Square Meter (m²)', 'Square km (km²)', 'Hectare (ha)', 'Are (a)', 'Square cm (cm²)', 'Square Inch (in²)', 'Square Feet (ft²)', 'Square Yard (yd²)', 'Acre (ac)', 'Square Mile (mi²)'],
  ...
  speed: {
    label: 'Kecepatan',
    units: ['m/s', 'km/h', 'mph', 'knot (kn)', 'ft/s', 'Mach (Ma)'],
  ...
  data: {
    label: 'Data Digital',
    units: ['Bit (b)', 'Byte (B)', 'Kilobyte (KB)', 'Megabyte (MB)', 'Gigabyte (GB)', 'Terabyte (TB)', 'Petabyte (PB)', 'Kibibyte (KiB)', 'Mebibyte (MiB)', 'Gibibyte (GiB)'],
  ...
  time: {
    label: 'Waktu',
    units: ['Nanosecond (ns)', 'Microsecond (μs)', 'Millisecond (ms)', 'Second (s)', 'Minute (min)', 'Hour (h)', 'Day (d)', 'Week (wk)', 'Month (mo)', 'Year (yr)', 'Decade', 'Century'],
  ...
  energy: {
    label: 'Energi',
    units: ['Joule (J)', 'Kilojoule (kJ)', 'Calorie (cal)', 'Kilocalorie (kcal)', 'Watt-hour (Wh)', 'kWh', 'eV', 'BTU', 'erg', 'Foot-pound (ft·lb)'],
  ...
  pressure: {
    label: 'Tekanan',
    units: ['Pascal (Pa)', 'Kilopascal (kPa)', 'Bar', 'Atmosphere (atm)', 'PSI', 'mmHg (torr)', 'inHg', 'Millibar (mbar)'],
...
function buildUnitSelects() {
  ...
  toSel.selectedIndex = Math.min(1, cat.ids.length - 1);
}
...
function convertUnit() {
  ...
  document.getElementById('unit-result').textContent = formatted + ' ' + to;
  document.getElementById('unit-formula').textContent = cat.formula ? cat.formula(from, to) : '';
...
function buildUnitTable() {
  ...
  tableEl.innerHTML = cat.ids.map((id, i) => {
    ...
    if (Math.abs(result) >= 1e10 || (Math.abs(result) < 1e-5 && result !== 0)) {
      fmt = result.toExponential(4);
    } else {
      fmt = parseFloat(result.toPrecision(8)).toString();
    }
    return `<div class="unit-chip">
      <div class="unit-chip-from">${cat.units[i]}</div>
      <div class="unit-chip-to">${fmt} ${id}</div>
    </div>`;
  }).join('');
}
